...
public class MulticorePerformanceModel extends PerformanceModel{
    ...
    public MulticorePerformanceModel(CompilationTask task, Context context, MulticoreProfileDataBase multicoreDB,
                                     Double multicoreClockPeriod, Double timeLimit) {
     ...
     */
    protected void restrictedGrowthConstraint(Map<Instance, DecisionVariables> instanceDecisionVariables,
                                            int numberOfCores) {



        GRBVar firstActorVariable = instanceDecisionVariables.get(getSoftwareActors().get(0)).getPartitionNumber();
        try {

            // -- the first actor can be assigned to either the first or the second core
            model.addConstr(firstActorVariable, GRB.EQUAL, 0.0,
            ...
            for (int instIx = 1; instIx < getSoftwareActors().size(); instIx ++) {
                ...
                GRBVar[] maxTerms = getSoftwareActors().subList(0, instIx).stream()
                        .map(i -> instanceDecisionVariables.get(i).getPartitionNumber()).toArray(GRBVar[]::new);
                GRBVar maxPrefix = model.addVar(0.0, numberOfCores - 1, 0.0,
                        GRB.INTEGER, "max_prefix_" + getSoftwareActors().get(instIx).getInstanceName());
            ...
            GRBVar numCoresUsed =  model.addVar(0.0, numberOfCores - 1, 0.0, GRB.INTEGER,
                    "num_used_cores");
            GRBVar[] allVarsArray = getSoftwareActors().stream()
                    .map(i -> instanceDecisionVariables.get(i).getPartitionNumber()).toArray(GRBVar[]::new);
...
//        ImmutableList<Connection> softwareConnections = getSoftwareOnlyConnections();
    ...
    protected GRBLinExpr getPartitionTimeExpression(SoftwarePartition partition,
        ...
        try {
            ...
            GRBVar[] decisionVariables = actors.stream().map(
            ...
            for (int ix = 0; ix < numActors; ix++)
                actorCost[ix] = this.multicoreDB.getInstanceTicks(actors.get(ix))
    ...
    private ImmutableList<Connection> getSoftwareOnlyConnections() {
        return task.getNetwork().getConnections().stream()
                .filter(connection -> {
                    Instance sourceActor = findInstance(connection.getSource());
                    Instance targetActor = findInstance(connection.getTarget());
                    return getSoftwareActors().contains(sourceActor) && getSoftwareActors().contains(targetActor);
    ...
    public ImmutableList<PartitioningSolution<String>> solveModel(int numberOfCores) {
        ...
        if (getMaxPartitions() < numberOfCores)
        ...
        try {
            ...
            Map<Instance, DecisionVariables> instanceDecisionVariables = getSoftwareActors().stream()
            ...
            Double partitionTimeUpperBound = getSoftwareActors().stream().map(
                    this.multicoreDB::getInstanceTicks
            ...
            GRBVar[] partitionExecutionTimeArray = partitionExecutionTimeList.toArray(
                    new GRBVar[0]);
            GRBVar executionTime = model.addVar(0.0, GRB.INFINITY, 0.0, GRB.CONTINUOUS,
                    "T_exec");
            ...
            for (SoftwarePartition partition: partitions) {
                ...
                GRBLinExpr localCommunicationTimeExpression = getLocalCoreCommunicationExpression(partition,
                        instanceDecisionVariables, getSoftwareOnlyConnections());
            ...
            GRBLinExpr globalCommunicationTimeExpression =
                    getCoreToCoreCommunicationTime(partitions, instanceDecisionVariables, getSoftwareOnlyConnections());
...
//        partitionTypes.sort(Comparator.comparingInt(SoftwarePartition::toIndex));
        ...
        for (TypedPartition p: partitionTypes) {
            ...
            ImmutableList<Instance> instancesInP = getSoftwareActors().stream()
                    .filter(inst -> {
    ...
    private void printTimingBreakdown(int id, PrintWriter writer) throws GRBException{
        ...
        printVar("T_lc");
        printVar("T_cc");
        writer.println(getVarByName("T") + "," + getVarByName("T_exec") + "," + getVarByName("T_lc") + "," + getVarByName("T_cc"));
